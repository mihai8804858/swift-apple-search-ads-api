...
public struct BudgetOrder: Decodable, Equatable, Sendable {
    ...
    /// Create an instance of `BudgetOrder`.
    ///
    /// - Parameters:
    ///     - id: A unique identifier for the budget order.
    ///     - parentOrgId: The unique identifier of the organization that owns the budget order.
    ...
    ///     - status: The system-controlled status indicator for the budget order.
    public init(
        id: Int,
        parentOrgId: Int,
