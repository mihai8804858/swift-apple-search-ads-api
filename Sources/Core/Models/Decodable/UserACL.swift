...
public struct UserACL: Decodable, Equatable, Sendable {
    ...
    /// Create an instance of `UserACL`.
    ///
    /// - Parameters:
    ///     - orgId: The identifier of the organization that owns the campaign.
    ///     - orgName: The name of your organization.
    ///     - parentOrgId: Distinguishes the account from an orgId belonging to a suborganization.
    ///     - currency: The organization’s default currency that is set up in the Apple Search Ads UI.
    ///     - paymentModel: The payment model that you set through the Apple Search Ads UI.
    ///     - roleNames: Each role governs what a user can see and do within the account.
    ///     - timeZone: The time zone you set during account creation through the Search Ads UI.
    public init(
        orgId: Int,
        orgName: String,
        parentOrgId: Int?,
